...
export default function Navbar() {
  ...
  return (
    <motion.nav
      className="fixed top-0 w-full z-50 border-b-4 border-[#C7984F] transition-all duration-500"
      style={{ fontFamily: "'Cormorant Garamond', serif" }}
      animate={{
        backgroundColor: scrolled ? 'rgba(43,26,16,0.97)' : 'rgba(59,42,26,1)',
        backdropFilter: scrolled ? 'blur(8px)' : 'blur(0px)',
      }}
      transition={{ duration: 0.4 }}
    >
      {/* Desktop navbar */}
      <div className="hidden md:flex max-w-8xl mx-auto items-center justify-center px-6 py-4 divide-x divide-[#F5F0EB]/25">

        {/* Left vine flourish */}
        <VineFlourishLeft />

        {/* Left links */}
        <ul className="flex-1 flex justify-end divide-x divide-[#F5F0EB]/25">
        ...
        <div className="flex items-center gap-5 px-7">
          <a href="https://github.com/Bjorno02" target="_blank" rel="noopener noreferrer"
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="#8FAE7E">
        ...
        {/* Right links */}
        <ul className="flex-1 flex justify-start divide-x divide-[#F5F0EB]/25">
      ...
      <div className="md:hidden flex items-center justify-between px-4 py-4">
        ...
        <button
          ...
          className="text-[#F5F0EB] p-1 bg-transparent border-none"
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
            {mobileOpen ? (
      ...
      {mobileOpen && (
        <motion.div
          className="md:hidden bg-[#3B2A1A] border-t border-[#C7984F]/30 px-4 pb-4"
          initial={{ opacity: 0, y: -8 }}
          animate={{ opacity: 1, y: 0 }}
            ...
            {allLinks.map((item) => (
              <li key={item.href}>
                <a
                  ...
                  className={`block py-2.5 px-3 rounded text-base tracking-wide ${
                    activeSection === item.href.slice(1)
                      ? 'text-[#C7984F] bg-[#4E3828]'
                      : 'text-[#F5F0EB]'
                  }`}
                  style={{ fontFamily: "'Cormorant Garamond', serif" }}
                >
